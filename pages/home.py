...
with col_cptec:
    if cptec["ok"]:
        ...
        <div style='background:{cptec['cor']}0d; border:1px solid {cptec['cor']}40;
                    border-radius:10px; padding:12px; min-height:100px; overflow:hidden'>
...
st.caption(
    f"Baseado na {fonte_label} · Altere o perfil acima para ver recomendações por segmento · "
    "\"Alta esperada em X meses\" = tempo até o preço reagir no mercado, não duração da escassez"
)
...
for i, (nivel, nome, icone_p, mensagem, horizonte, regiao) in enumerate(produtos_status):
    ...
    rodape = (
        ...
        f"Alta esperada em ~{horizonte} meses &nbsp;·&nbsp; {regiao}</div>"
